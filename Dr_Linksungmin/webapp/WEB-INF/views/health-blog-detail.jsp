						...
						<div class="col-lg-8 col-md-12">
							<div class="blog-view">
								<div class="blog blog-single-post">
									...
									<c:otherwise>
										<div class="blog-image">
										<a href="javascript:void(0);"><img alt="" src="${pageContext.request.contextPath}/resources/img/blog/${n_board.news_photo}.png" class="img-fluid"></a>
									</div>
									</c:otherwise>
									</c:choose>
									...
									<div class="blog-info clearfix">
										<div class="post-left">
											<ul>
												<li><i class="far fa-calendar"></i>${n_board.news_regdate }2020.12.03</li>
												<li><i class="far fa-comments"></i>${n_board.getCnt }12 Comments</li>
											</ul>
										</div>
									</div>
...
급성 인대 손상인 발목 염좌의 치료는 비수술적인 치료가 원칙입니다. 초기에는 부종과 통증을 줄이기 위해서 발목을 고정하고, 심장보다 20-30cm 높게 다리 올리기, 압박드레싱, 냉찜질 등을 시행합니다. 고정 방법(보호대, 반기브스, 부츠, 통기브스 등)과 고정 기간은 손상 정도에 따라 짧게는 2-3주 이내에서 길게는 4-6주까지도 시행하게 되며, 통증과 붓기가 줄어들면 점차 체중부하 및 보행 연습을 하게 됩니다. 발목의 기능적인 회복을 돕기 위해 발목 외측 근육인 비골근 강화운동과 고유감각 훈련을 함께 병행합니다. 고유감각이란 자세, 위치, 움직임 등에 대한 감각을 말하는데, 발에 대한 위치 감각이 감소하게 되면 발을 지면에 바로 놓지 못하고 잘못된 위치로 발을 디디면서 발을 접지르는 빈도가 증가하게 됩니다. 따라서 발목 염좌에서의 고유감각 훈련은 발의 위치에 대한 감각을 강화하기 위한 운동으로서 주로 균형을 잡는 훈련으로 구성됩니다.</p>
								...
								<div class="card blog-comments clearfix">
									...
									<input type="hidden" value="${n_board.news_board_num}" name="b_num"> 
									...
									<ul class="comments-list">
										...
										<c:forEach var="nr" items="${n_repl}">
											<div class="comment">
												...
												<c:forEach items="${nr.patients}" var="p">
													<span class="blog-author-name">${p.p_id }</span>
												</c:forEach>
												<c:forEach items="${nr.doctors}" var="d">
													<span class="blog-author-name">${d.d_id }</span>
												</c:forEach>												
													</span>
													<c:forEach var="repl" items="${nr.news_repl_comment }">
													<p>${repl }겨울철엔 날씨도 갑자기 추워지고 일교차도 심해져서 건강이 약하거나 평소에 몸이 좋지 않은 사람들에게는 위험할 수 있는데 이 글을 추천해 주어야겠네요</p>
													</c:forEach>
													<p class="blog-date">${nr.n_repl_date }Dec 6, 2017</p>
